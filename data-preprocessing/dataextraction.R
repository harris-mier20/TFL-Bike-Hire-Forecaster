...
# creting a blank dataset to be filled with only the stations within the predifned postcode list
...
### reformats dlpr date format into the fromat required to call the csv###
...
### pulling thestart date out of link###
...
olddataformatextraction <- function(startinglink){
  ...
  for (x in 1:((335-startnum)+1)) {
    ...
    if (x != 206){
      ...
      #Chanig colmun names to match new format
